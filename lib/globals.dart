...
//String ServerURL = "192.169.188.163";
String ServerURL = "moizfoods.kale-labs.com";
//String lServerURL = "192.168.201.155:8080";
//String ServerURL = "192.168.201.163:8081";
//Almoix current server ip"192.169.188.163"; //"18.193.86.188"; new server 52.59.79.170
//String ServerURL = "192.168.201.144:8080";
//String ServerURL = "52.59.79.171"; //"18.193.86.188";
//String ServerURLLocal = "52.59.79.171:8080";
//String ServerURL="http://portal.pbc.com.pk/portal";
String DeviceID = ""; //"""fc5cc47e99573fc";
...
int city_id=0;
int pjpid=0;
// Define a boolean variable to control the visibility of the leading icon
int brand_id = 0;
int UserID = 0;
...
//To Start the Visit
String Start_Visit_Time = "";
//To End the Visit
String End_Visit_Time = "";
//
int isAlternative = 1;
...
String Cities = "";
int  City_Id = 0;
int outletImageTypeId = 0;
int isSpotSale = 0;
int PciChannelID = 0;

String newOutletId = "";
String newOutletLat = "0.0";
String newOutletLng = "0.0";
String newOutletAcc = "0.0";
...
Future<int> asyncImagesUpload() async {
  //create multipart request for POST or PATCH method
  int countImages=0, successImages=0;
  print("upload started--Fahad");
  Repository repo=Repository();
  ...
  await repo.getRegisterOutletImages().then((val) async {
    for (int i = 0; i < val.length; i++) {
      ...
      try {
        ...
        var url = Uri.https(ServerURL, '/portal/mobile/MobileUploadNewOutletImage');
        ...
        request.fields['value2'] = UserID.toString(); //UserId.toString();
  ...
  await repo.getAddedChnnelTggingImages().then((val) async {
    ...
    for (int i = 0; i < val.length; i++) {
      ...
      try {
        ...
        //alfalah_portal
        var url = Uri.https(ServerURL, '/portal/mobile/MobileUploadOutletImage');
        print('URL ' + url.toString());
  ...
  print("Total Images : "+countImages.toString());
  print("successImages : "+successImages.toString());
  if(countImages == successImages){
    IsImagesUpload = 0;
  }
  return (countImages - successImages);
}
...
void startContinuousLocations(context) async {
...
void startContinuousLocation1(context) async {
  if (locationStream == null || locationStream.isPaused == true) {
    if (await Permission.location.request().isGranted) {
      if (await Permission.location.serviceStatus.isEnabled) {
        // Either the permission was already granted before or the user just granted it.
...
void startContinuousLocationUser(context) async {
  print("Call Init Loc");
/*
  try {
    print("Call Init Loc 2");
          ...
          actions: <Widget>[
            ...
            new ElevatedButton(
              ...
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => UserAttendance()),
                );
              },
            ),
          ],
    ...
    if (await Permission.location.serviceStatus.isEnabled) {
      ...
      if (await Geolocator.isLocationServiceEnabled()) {
        ...
        locationStream = Geolocator.getPositionStream(
            ...
            forceAndroidLocationManager: true)
            .listen((Position position) {
          latAtt = position.latitude;
          lngAtt = position.longitude;
          accuracyAtt =  position.accuracy;
          print(position == null
              ? 'Unknown'
              : position.latitude.toString() +
              ', ' +
