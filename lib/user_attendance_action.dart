...
import 'package:order_booker/AppString.dart';
...
class UserAttendanceAction extends StatefulWidget {
  @override
  _UserAttendanceActionState createState() => _UserAttendanceActionState();
}

class _UserAttendanceActionState extends State<UserAttendanceAction> {
...
//    imagePath = (await new ImagePicker().getImage(source: ImageSource.camera, imageQuality: 30, preferredCameraDevice: CameraDevice.rear));
  ...
  void initState() {
    ...
    globals.startContinuousLocationUser(context);
  }
...
/*  Future<void>  GetLocation() async {
  ...
  Future getCurrentLocation(context) async {
    //Old code for location Permission
    // if (await Permission.location.request().isGranted) {
    //   if (await Permission.location.serviceStatus.isEnabled) {
    // Either the permission was already granted before or the user just granted it.
    PermissionStatus location = await Permission.location.request();

    var positione = await GeolocatorPlatform.instance
        .getCurrentPosition(desiredAccuracy: LocationAccuracy.medium);
    return positione;
  }
  Future MarkAttendanceLocally() async {
    ...
    var url = Uri.https(globals.ServerURL,
        '/portal/mobile/MobileSyncMarkUserAttendance');
      ...
      if (internetCheck == 1) {
        try {
          ...
          if (responseBody["success"] == "true") {
            ...
            _showDialog(AppStrings.error, responseBody["error"], 1);
          } else {
            _showDialog(
                "Error",
                responseBody["error_code"] + ": Attendance Not Upload",
                1);
          }
        } catch (e) {
          //  print('e ==> '+e.toString());
            ...
            children: <Widget>[
              ...
              RawMaterialButton(
                // fillColor: Colors.teal,
                  ...
                  onPressed:  () async => _image != null && imagePath != null
