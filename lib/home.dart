...
class _Home extends State<Home> {
  ...
  void initState() {
    ...
    repo.getTotalOrders().then((value) {
      setState(() {
        totalOrders = value;
        totalVisits = totalOrders + totalNoOrders;
  ...
  Future _UploadOutletMarkClosed() async {
    ...
    await repo.getAllOutletMarkClose(0).then((val) async {
      ...
      for (int i = 0; i < AllOutletsMarkedClose.length; i++) {
        ...
        var localUrl =
            "http://192.168.30.125:8080/portal/mobile/MobileSyncOutletClosed";
        var url = Uri.https(
    ...
    await repo.getAllOutletMarkClose(1).then((val) async {
      ...
      for (int i = 0; i < AllOutletsMarkedClose.length; i++) {
        ...
        try {
          ...
          var localUrl =
              "http://192.168.30.125:8080/portal/mobile/MobileUploadOutletClosedImage";
...
//      Wave/grain/sales/MobileVFSalesContractExecute
      ...
      if (response.statusCode == 200) {
        if (responseBody["success"] == "true") {
          ...
          for (var i = 0; i < stock.length; i++) {
           await repo.insertStockPosition(stock[i]['ProductID'],
  ...
  //       };
  //       var localUrl =
  //           "http://192.168.30.125:8080/portal/mobile/MobileSyncMarkAttendanceV3";
  //       var url = Uri.http(
      ...
      for (int i = 0; i < AllMerchandsingPhotos.length; i++) {
        ...
        try {
          ...
          var stream =
              new http.ByteStream(DelegatingStream.typed(photoFile.openRead()));
          var length = await photoFile.length();
          //   var localUrl = Uri.http(globals.ServerURLLocal,"/nisa_portal/mobile/MobileUploadOrdersImageV2");
          var url = Uri.https(
  ...
  Future _UploadNoOrder(context) async {
    ...
    await repo.getAllNoOrders(0).then((val) async {
      ...
      for (int i = 0; i < AllNoOrders.length; i++) {
        ...
        var localUrl =
            "http://192.168.30.125:8080/portal/mobile/MobileSyncNoOrdersV2";
        var url =
      ...
      for (int i = 0; i < AllRegisteredOutlets.length; i++) {
        ...
        try {
          ...
          if (response.statusCode == 200) {
            if (responseBody["success"] == "true") {
              print("Saved");
              repo.markOutletUploaded(AllRegisteredOutlets[i]['mobile_request_id']);
              //Navigator.of(_keyLoader.currentContext,rootNavigator: true).pop();
  ...
  Future _UploadOrder(context) async {
    ...
    await repo.getAllOrdersByIsUploaded(0).then((val) async {
      ...
      for (int i = 0; i < AllOrders.length; i++) {
        ...
        var url =
            Uri.https(globals.ServerURL, '/portal/mobile/MobileSyncOrdersV10');
        print(url);
              ...
              children: <Widget>[
                        ...
                        .whenComplete(() => _OutletRegisterationUpload(context)
                           // .whenComplete(() => _SyncMarkedAttendance()
                                .whenComplete(() => _UploadOutletMarkClosed()
                                    .whenComplete(() => isUploaded == 1 &&
                                            isAnythingUploaded == 1
                                        ? _showDialogFinalMessage("Success", "Data Uploaded")
                                        : "")
                ...
                ListTile(
                  ...
                  onTap: () async {
                    ...
                    if(countImages != 0){
                      Dialogs.showLoadingDialog(context, _scaffoldKey2);
                      int yy = await globals.asyncImagesUpload();
                     // print("yy after "+yy.toString());
                      setState(()  {
                        countImages = yy;
                      });
                     // print("countImages after "+countImages.toString());

                      Navigator.of(context, rootNavigator: true).pop('dialog');
                      _showDialog("Success", "Images Uploaded");
                    }else{
                      Navigator.of(context, rootNavigator: true).pop('dialog');
                      _showDialog("Warning", "No Images Found");
                    }
                    /*if (globals.IsImagesUpload == 0) {
                      print("uploading file");
                      globals.asyncImagesUpload();
                    } else {
                      print("waiting......");
                    }*/
