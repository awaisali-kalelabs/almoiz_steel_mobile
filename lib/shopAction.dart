...
/*    timer = Timer.periodic(Duration(seconds: 2), (Timer timer) {
  ...
  void someAsyncMethod() async {
    ...
    // You can await Newdistance wherever you need to use its value
     actualDistance = await Newdistance;
    print('Distance=====> $actualDistance meters');
  }
  ...
  Future _EndVisit() async {
    ...
    await repo.getVisitTime().then((val) async {
      ...
      "&distance_from_store=" +
        ...
        var url =
        Uri.https(globals.ServerURL, '/portal/mobile/MobileSyncOutletVisitDurationV2');
        print(url);
        print(orderParam);
                  ...
                  children: <Widget>[
                    Column(
                      children: [
                        ...
                        Divider(height: 10,),
                        Visibility(
                          visible: ShowDistance, // Set visibility based on the boolean variable
                          child: Container(
                            height: 200,
                            child :Text(
                              'You are ${actualDistance.toStringAsFixed(2)} m away from the Outlet',
