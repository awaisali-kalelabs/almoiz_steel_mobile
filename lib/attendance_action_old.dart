...
class AttendanceActionOld extends StatefulWidget {
  @override
  _AttendanceActionState createState() => _AttendanceActionState();
}

class _AttendanceActionState extends State<AttendanceActionOld> {
...
//    imagePath = (await new ImagePicker().getImage(source: ImageSource.camera, imageQuality: 30, preferredCameraDevice: CameraDevice.rear));
  ...
  MarkAttendanceLocally() async {
    ...
    print(globals.getUniqueMobileId());
  ...
  Future _UploadMarkAttendance() async {
    ...
    await repo.getAllMarkedAttendances(0).then((val) async {
      setState(() {
        ...
        print("AllMarkedAttendances" + AllMarkedAttendances.toString());
      for (int i = 0; i < AllMarkedAttendances.length; i++) {
        ...
        // var url = Uri.http(
        //     globals.ServerURL, '/portal/mobile/MobileSyncMarkAttendanceV2');

        var url = Uri.https(
            globals.ServerURL, '/portal/mobile/MobileSyncMarkAttendanceTest');
        ...
        try {
          ...
          if (response.statusCode == 200) {
            if (responseBody["success"] == "true") {
              ...
              Navigator.of(context, rootNavigator: true).pop('dialog');
              _showDialog("Fail", responseBody["error"], 1);
              print("fail" + responseBody["error"]);
              await repo.markAttendanceUploaded(ORDERIDToDelete);
            }
          } else {
