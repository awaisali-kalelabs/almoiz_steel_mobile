...
void main() async {
  ...
  runApp(AddToCart(globals.OutletID));
}
...
class _AddToCart extends State<AddToCart> {
  ...
  Future GetOutletOrderItemInfo() async {
    ...
    print("======"+globals.brand_id.toString());
    OutletOrder =
        await repo.getOrderItemInfo(globals.orderId, globals.productId as int);
          ...
          setState(() {
            if (value == null) {
              ...
              discountController.text =
                  defaultDiscount == null ? "0" : defaultDiscount.toString();
            } else {
  ...
  Future<void> addItemOrder(orderId, productId, discount, quantity, amount,
      createdOn, rate, productLabel) async {
    var source_id = globals.getUniqueMobileId();
    List Items = new List();
    Items.add({
    ...
    print("isNewEntry" + isNewEntry.toString());
    List<Map<String, dynamic>> product = await repo.getProductById(productId);
    List<Map<String, dynamic>> promotionalProduct =
        await repo.getAllPromotionalProduct(
            globals.OutletID, product[0]['package_id'], product[0]['brand_id']);
    int promotionId = await repo.getPromotionId(
        globals.OutletID, product[0]['package_id'], product[0]['brand_id']);
    if (promotionalProduct != null && promotionalProduct.isNotEmpty) {
      int unitsApplicable = promotionalProduct[0]['total_units']; //2
      int quantityUnits = product[0]['unit_per_case'] * quantity; //1
      int freeUnits = 0;
      ...
      if (freeUnits > 0) {
        ...
        List<Map<String, dynamic>> freeProduct =
            await repo.getPromotionProductsFree(promotionId);
        ...
        product = await repo.getProductsByPackageIdAndBrandId(
            freeProduct[0]['package_id'], freeProduct[0]['brand_id']);
        Items = new List();
        Items.add({
          ...
          'unit_quantity': freeProduct[0]['total_units'],
          'is_promotion': 1,
          'promotion_id': promotionId,
          'id': globals.getUniqueMobileId(),
          'source_id': source_id
        });
       await repo.addItemToCurrentOrder(orderId, Items, isNewEntry);
      }
    }
  }
  ...
  onDiscountChange(val) {
    ...
    /* if (double.parse(discountController.text) > maximumDiscount) {
      errorMessage =
          "Discount cannot be greater than " + maximumDiscount.toString() + "";
    }*/
    if (double.parse(discountController.text) < priceRate) {
      //&& double.parse(discountController.text) <= maximumDiscount
      priceRateAfterDiscount =
          priceRate - double.parse(discountController.text);
    } else {
      Flushbar(
        messageText: Column(
          children: <Widget>[
    ...
    if (int.parse(quantityController.text) != 0) {
      double amount = 0;
      double price =
          double.parse(quantityController.text) * priceRateAfterDiscount;
                                ...
                                child:TextFormField(
                                  ...
                                  onChanged: (val) {
                                    repo
                                      ...
                                      print(priceRate);
                                      print(value);
                                      double discount = ((priceRate * value) /
                                          100); //*int.parse(val)
                                      setState(() {
                                      ...
                                      onDiscountChange(
                                          discountController.text);
                                      print("priceRateAfterDiscount" +
                                          priceRateAfterDiscount.toString());
                                    ...
                                    repo.getminQuantity(
                                      ...
                                      setState(() {
                                        MinimumQuantity = value[0]['MinimumQuantity'].toString();
                                        print("MinimumQuantity"+MinimumQuantity);
                                      });
                                    });
                                  },
                                  validator: (val) {
                                    ...
                                    if (MinimumQuantity != null && MinimumQuantity.isNotEmpty) {
                                      // Check if MinimumQuantity is a valid integer
                                      int minQuantityInt = int.tryParse(MinimumQuantity);

                                      if (minQuantityInt != null && minQuantityInt > 0) {
                                        try {
                                          int valInt = int.parse(val);

                                          if (valInt < minQuantityInt) {
                            ...
                            child: InkWell(
                              onTap: () {
                                if (_formKey.currentState.validate()) {
                                  ...
                                  String errorMessage =
                                      "Discount cannot be greater than rate";
