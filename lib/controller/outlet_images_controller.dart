...
class PerformTask extends GetxController {
  ...
  Future<void> saveImageDataLocally() async {
  ...
  Future<void> submitData() async {
    ...
    int orderID =commonFunctions.orderId.value;
...
/*    var currDate = new DateTime.now();
    ...
    await dbController.getOutletNoOrders().then((val) async {
      ...
      for (int i = 0; i < AllNoOrders.length; i++){
        String orderParam = "MobileTransactionNumber=${AllNoOrders[i]['id']}"
            "&PJPID=1"
            "&MobileTimestamp=$TimeStamp"
            "&ReasonID=1&"
            "OutletID=${AllNoOrders[i]['outlet_id']}"
            "&UserID=${commonFunctions.userId}&"
            "Lat=${commonFunctions.latitude.value}&"
            "Lng=${commonFunctions.longitude.value}&"
            "Accuracy=22.00555&UUID=852852&"
            "Platform=android&"
            "AppVersion=1.0&"
            "RegionID=2&"
            "CityID=4"
            "&DeviceID=123154354134524";
