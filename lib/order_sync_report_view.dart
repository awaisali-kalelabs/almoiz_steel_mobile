...
class OrdersSyncReportView extends StatefulWidget {
  int OrderId = 0;

  OrdersSyncReportView({int OrderId}) {
    this.OrderId = OrderId;
  }

  @override
  _OrdersSyncReportView createState() => _OrdersSyncReportView(OrderId);
}

class _OrdersSyncReportView extends State<OrdersSyncReportView> {
  ...
  _OrdersSyncReportView(int OrderId) {
    this.OrderId = OrderId;
  }
  ...
  void initState() {
    ...
    repo.initdb();
    repo.getAllOrdersForSyncReport().then((value) => {
          setState(() {
            SyncPosition = value;
          })
        });
...
//        .addPostFrameCallback((_) => getSyncPosition());
  ...
  void getSyncPosition() async {
    ...
    try {
      var url = Uri.https(globals.ServerURL, '/portal/mobile/MobileSyncReport',
          QueryParameters);
      //var url = Uri.http("192.168.30.125:8080", '/nisa_portal/mobile/MobileSyncReport', QueryParameters);
...
Future<void> uploadOrder222(String orderParam) async {
  ...
  // var url = Uri.http(globals.ServerURL,  '/nisa_portal/mobile/MobileSyncOrdersV9', QueryParameters);
//      Wave/grain/sales/MobileVFSyncContractExecute
  var url = Uri.https(globals.ServerURL, '/portal/mobile/MobileSyncOrdersV9');
  print(url);
  var response = await http.get(url, headers: {
    HttpHeaders.contentTypeHeader: 'application/x-www-form-urlencoded'
  });
